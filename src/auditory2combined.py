...
pw = (times>=80)&(times<=130)
top = np.argsort(np.abs(evoked[:,pw]).max(axis=1))[::-1][:4]
n_pre = int(0.1*sf)
post = np.abs(epochs[:, :, (times>=80)&(times<=130)]).mean(axis=(1,2)) 
pre  = np.abs(epochs[:, :, times<0]).mean(axis=(1,2))                   
from scipy import stats
t,p = stats.ttest_rel(post, pre)
print(f"post-stim vs baseline: t={t:.2f}, p={p:.4f}")
print(f"mean post {post.mean()*1e15:.1f} fT vs pre {pre.mean()*1e15:.1f} fT")
rms = np.sqrt((evoked**2).mean(axis=0))     
gfp = np.sqrt((evoked**2).mean(axis=0))         
baseline_gfp = gfp[times < 0]                  
peak_gfp = gfp[(times>=80)&(times<=130)].max()    
print(f"peak GFP {peak_gfp*1e15:.0f} fT vs baseline mean±sd "
      f"{baseline_gfp.mean()*1e15:.0f} ± {baseline_gfp.std()*1e15:.0f} fT")
print(f"peak is {(peak_gfp-baseline_gfp.mean())/baseline_gfp.std():.1f} SDs above baseline")
...
plt.plot(times, rms*1e15)
plt.axvline(0,color="k",ls="--"); plt.axvline(100,color="r",ls=":")
plt.xlabel("ms from tone"); plt.ylabel("RMS field (fT)")
